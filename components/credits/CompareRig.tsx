...
export function CompareRig({
  ...
  const [active, setActive] = useState<string>(lanes[0]?.id ?? "");
  const [playing, setPlaying] = useState(false);
  const [time, setTime] = useState(0);
  const [duration, setDuration] = useState(0);
  const [armed, setArmed] = useState(false);
  const [envs, setEnvs] = useState<Record<string, WaveformEnvelope | null>>({});
  ...
  const refFor = useCallback((id: string) => audioRefs.current.get(id) ?? null, []);
  ...
  useEffect(() => {
    const draw = () => {
      lanes.forEach((lane) => {
        drawLane(canvasRefs.current.get(lane.id) ?? null, envs[lane.id] ?? null, active === lane.id);
      });
    };
    draw();
  ...
  const arm = useCallback(() => {
    ...
    setArmed(true);
    lanes.forEach((lane) => {
      const el = audioRefs.current.get(lane.id);
      if (el) {
        el.preload = "auto";
        el.load();
      }
      void decodeWaveformEnvelopeFromUrl(lane.src).then(
        (e) => e && setEnvs((prev) => ({ ...prev, [lane.id]: e }))
      );
    });
    ...
    try {
      ...
      lanes.forEach((lane) => {
        ...
        analysers.current.set(lane.id, an);
      });
    } catch {
      ...
      if (el) {
        ...
        if (lp && t >= lp.end) {
          audioRefs.current.forEach((a) => {
            a.currentTime = lp.start;
          });
        }
        setTime(el.currentTime);
      }
      const an = analysers.current.get(activeRef.current);
      let peak = 0;
      if (an) {
  ...
  function onLanePointerDown(id: string, e: React.PointerEvent<HTMLDivElement>) {
    if (!duration) {
      switchTo(id);
      return;
    }
  ...
  function onLanePointerUp(id: string, e: React.PointerEvent<HTMLDivElement>) {
    if (!drag) return;
  ...
  function onKey(e: React.KeyboardEvent) {
    ...
    if (Number.isInteger(idx) && idx >= 1 && idx <= lanes.length) {
      switchTo(lanes[idx - 1].id);
      return;
    }
    switch (e.key) {
      case " ":
    ...
    >
      {lanes.map((lane) => (
        <audio
          key={lane.id}
          ref={(el) => {
            if (el) audioRefs.current.set(lane.id, el);
            else audioRefs.current.delete(lane.id);
          }}
          src={lane.src}
          preload="metadata"
      ...
      <div className="flex items-center gap-3 border-b border-graphite-800 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium text-text-primary">{headline}</p>
          <p className="truncate text-xs text-text-subtle">
            {loop ? (
              ...
              subline ?? "Click a lane to switch. Drag on a lane to loop a section."
            )}
          </p>
        </div>
      ...
      <div ref={lanesRef} className="relative divide-y divide-graphite-800">
        {lanes.map((lane, i) => {
          ...
          return (
            <div
              key={lane.id}
              className={cn("relative flex select-none", laneHeight, isActive ? "bg-amber-500/[0.04]" : "bg-transparent")}
            >
              <button
                type="button"
                role="radio"
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-[13px] font-medium leading-tight">{lane.title}</span>
                </span>
                {lane.note && (
                  <span className="mt-0.5 pl-3.5 text-[10px] leading-tight text-text-subtle">{lane.note}</span>
                )}
                {isActive && (
                  <span className="mt-1.5 ml-3.5 h-1 w-14 overflow-hidden rounded-full bg-graphite-800" aria-hidden>
                    <span
              ...
              >
                <canvas
                  ref={(el) => {
                    ...
                    else canvasRefs.current.delete(lane.id);
                  }}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-2.5 text-[11px] text-text-subtle">
        <span>Same clip. Switching keeps the playhead.</span>
        <span className="hidden font-mono sm:inline">Space play · 1-{lanes.length} switch · &larr; &rarr; seek · L clear loop</span>
      </div>
    </figure>
  );
}
