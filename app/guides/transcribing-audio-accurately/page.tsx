...
const articleJsonLd = {
  ...
  url: `${SITE_URL}/guides/${guide.slug}`,
  // Kept as the fuller WebPage form rather than a bare string. Both are valid;
  // this one ties the Article to this exact URL explicitly, so a syndicated or
  // scraped copy isn't as strong a candidate for the schema as the original.
  mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guides/${guide.slug}` },
  image: `${SITE_URL}${OG_IMAGE.url}`,
  publisher: { "@type": "Organization", name: "AudioForges" },
};

// FAQSection emits the FAQPage schema from this array, so the visible answers
// and the structured data can't drift apart. These are written as standalone
// answers — a rich result shows them without the surrounding page, so each has
// to make sense cold.
const faqs: FAQItem[] = [
  {
      ...
      <main id="main" className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
        ...
        <Prose className="mt-10">
          <p>
            ...
            separates those two outcomes is under your control.
          </p>

          <h2 id="what-degrades">What actually degrades accuracy</h2>
          <p>
            A few specific conditions reliably cause more errors, regardless of
            which engine is doing the work:
          </p>
          <dl>
            <dt>Background noise</dt>
            <dd>
              Hiss, hum, or ambient sound competing with the voice makes it harder
              to isolate speech from everything else in the signal.
            </dd>

            <dt>Overlapping speech</dt>
            <dd>
              Two people talking at once is genuinely difficult for any
              transcription system, since it has to separate simultaneous voices
              rather than recognize one continuous stream.
            </dd>

            <dt>Low recording volume or clipping</dt>
            <dd>
              Audio that&apos;s too quiet, or distorted from being too loud, both
              reduce the clarity available to work with.
            </dd>

            <dt>Heavy accents or unclear speech</dt>
            <dd>
              Mumbled, fast, or heavily accented delivery is harder than clearly
              enunciated speech, though this narrows considerably with larger
              models.
            </dd>

            <dt>Music under the voice</dt>
            <dd>
              A podcast intro bed or a busy field recording competes directly with
              the speech you want.
            </dd>
          </dl>
          ...
          <p>
            Background noise is both the most common cause of transcription errors
          ...
          <p>
            If music is the problem rather than noise, separating the vocal first
            is more effective than any denoiser: a{" "}
            <Link href="/vocal-remover">vocal removal</Link> pass gives you a
            speech-only track to transcribe.
          </p>
          ...
          <p>
            ...
            opens with English pleasantries before switching to another language
            will be labelled English. Two languages alternating throughout will be
            assigned whichever one happened to come first.
          </p>
          <p>
          ...
          <h2 id="transcribe-vs-translate">Transcribing versus translating</h2>
          <p>
          ...
          <p>
            ...
            available. Spanish audio can become Spanish text or English text, but
            not French text. For any other pairing, transcribe in the source
            language and translate the finished text separately.
          </p>
        </Prose>

        {/* Three genuine columns of the same dataset — a table, not a list.
            Sits outside Prose so it can use the full column width. */}
        <section className="mt-12">
          <h2
            id="which-export"
            className="measure text-2xl font-bold tracking-[-0.01em] text-text-primary"
          >
          ...
          <Prose className="mt-5">
            <p>
              ...
              reason a caption file silently fails to load. If an editor accepts
              your file but shows no captions, that&apos;s the first thing to
              check.
            </p>
          </Prose>
        </section>
        ...
        <Prose className="mt-12">
          ...
          <p>
            One thing to watch when reassembling: timestamps restart from zero in
            each section&apos;s transcript. Joining plain text is straightforward,
            but building captions from split sections means adding each
            section&apos;s start offset to its timings first.
          </p>

          <h2 id="speed">What to expect on speed</h2>
          <p>
            Transcription runs on a GPU worker that spins down when it&apos;s
            idle. In practice that means the wait is dominated by startup rather
            than by the length of your file: the first run after a quiet period
            takes about a minute to begin, and once the worker is warm a
            ten-minute recording finishes in well under a minute.
          </p>
          <p>
            The counterintuitive consequence is that a thirty-second voice memo
            and a ten-minute podcast often take roughly the same wall time. If a
            short clip seems to be taking a while, that&apos;s almost always a
            cold start rather than a problem with your file.
          </p>
          ...
          <ol>
            ...
            <li>
              ...
              <Link href="/voice-clean">Voice Cleaner</Link>, or the{" "}
              <Link href="/noise-remove">Noise Remover</Link> when you want manual
              control over how aggressive it is.
            </li>
            <li>Set the language if the clip is short, accented, or mixes languages.</li>
            <li>
              Transcribe with <Link href="/audio-to-text">Audio to Text</Link>, or
              paste a link into{" "}
              <Link href="/youtube-to-text">YouTube to Text</Link> to skip the
          ...
          <p>
            ...
            <Link href="/video-to-text">Video to Text</Link> takes MP4, MOV, MKV
            and WEBM directly, so there&apos;s no need to extract the audio track
            first.
          </p>
        </Prose>
